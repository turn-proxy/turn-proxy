...
- **The ~5-min TURN death is VK expiring the nonce on *idle* allocations, not VK refusing refresh — kept alive by the heartbeat.** pion creates a permission and binds a channel per peer on first write, then refreshes them on timers (permission 120s, channel-bind 5 min). VK's TURN is non-standard about stale nonces: it **expires the nonce after <60s of allocation idleness**, and when a refresh arrives with the stale nonce it answers `438 Wrong nonce` on the allocation *Refresh* but a mis-coded `400 Bad Request` (`Fail to refresh permissions` / `Failed to bind channel`) on *CreatePermission*/*ChannelBind* — and once stale, VK rejects even its own freshly-issued nonce (`400`), so pion's built-in 438-retry can't recover. So the fix is **preventive, not reactive: the 5s heartbeat keeps the allocation warm** (any relayed traffic resets VK's idle-nonce timer), so the nonce never goes stale and pion's native permission/channel refreshes succeed. **Wire-capture proof (live VK P2P TURN, `turn:95.163.34.136:19302`): libwebrtc reuses one nonce and refreshes permissions/channels at t+240s & t+480s all SUCCESS while actively passing keepalive; a standalone pion against the same server+creds fails *only* on an idle allocation, and succeeds with a 5s keepalive.** This **corrects** the earlier "VK rejects refresh entirely" note. The old workarounds — suppressing the permission refresh (`PermissionRefreshInterval` 30 min) and a 3-min proactive reconnect — were **removed as redundant**; pion now refreshes at its defaults and the heartbeat warms the nonce. (Caveat: verified for the permission refresh; the channel-bind refresh at 5 min uses the same warm nonce and should likewise succeed — confirm on a real server+client deployment.) The reference's plain UDP mode (`oneTurnConnection`) has the same silent-stall hole — only its KCP/VLESS mode (`maintainVLESSSession`) self-heals via KCP keepalive; our heartbeat is that mechanism without KCP
- **Stream liveness (client): heartbeat + stall-detect + proactive reconnect** (`obfs/srtp.go`, `client/stream_pool.go`). Each stream's `relayStreamToLocal` runs a watcher goroutine that every `heartbeatInterval` (5s) sends a `FrameHeartbeat` SRTP packet (PT 110, 1-byte payload) and checks `tunnel.LastInbound()`; if nothing (data *or* heartbeat echo) has arrived for `stallTimeout` (15s) it closes the tunnel → the supervisor re-allocates. The **server echoes** heartbeats: `NewSRTPConn(..., echo=true)` makes `SRTPConn.Read` bounce a `FrameHeartbeat` straight back instead of forwarding it to WG; the client constructs with `echo=false`. The echo rides the **same allocation** (point-to-point per VK relay IP), so it returns on the sending stream regardless of how work-stealing spreads WG payload. `SRTPConn` tracks `lastInbound` (atomic, set on every decrypted frame) and serializes all sends through a mutex (data writes and heartbeat echoes race otherwise). The 5s heartbeat doubles as the **TURN nonce keepalive** (its relayed traffic resets VK's idle-nonce timer, see Key constraints), so streams hold a single long-lived allocation and pion's native refreshes succeed — there is **no** proactive per-stream reconnect; a stream only re-allocates on a real stall (`stallTimeout`) or error. Far fewer allocations are created/closed, which also relieves the 486 quota
...
- `internal/client/stream_pool.go` — `streamPool`: the shared per-run stream state (local conn, queue, `targetResolver` handle, transport). `newStreamPool(local, targets, transport)` — no `config.Config`. Per-stream methods `supervise`/`runStream` take just `(ctx, idx)`; also `readLocal` and the `relayStreamToLocal` work-stealing pump, whose watcher goroutine drives the heartbeat send + stall-detect (see Stream liveness; consts in `client.go`)
